...
export default function Contact() {
  ...
  >("idle");
  const { sectionTitle, sectionSubtitle, formLabels, footer } =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "https://formsubmit.co/ajax/tho.deb.prof@gmail.com",
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <form className="space-y-16" onSubmit={handleSubmit}>
          ...
          >
            ...
            <span className="relative z-10 font-display text-2xl md:text-5xl font-black uppercase tracking-[0.2em] flex items-center gap-6">
              ...
              {status === "success" && (
                <>
                  <CheckCircle2 className="w-8 h-8 md:w-12 md:h-12" /> Signal
                  Reçu
                </>
              )}
              {status === "error" && (
                <>
                  <XCircle className="w-8 h-8 md:w-12 md:h-12" /> Échec
                </>
              )}
              {status === "idle" && (
                <>
        ...
        <div className="mt-24 pt-8 border-t border-dune-bg/20 flex flex-col md:flex-row justify-between items-center gap-8 md:gap-4">
          ...
          <div className="flex items-center gap-6 md:gap-12">
            {portfolioContent.socials.map((social, index) => (
              <a
                ...
                className="font-body text-[10px] font-bold tracking-[0.3em] uppercase text-dune-bg hover:text-dune-spice transition-colors"
              >
                {social.name}
              </a>
            ))}
